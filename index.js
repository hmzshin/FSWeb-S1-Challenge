...
console.log(`dizinin birinci ojesinin profili: ${fenomenler[0].profile}`);

//(2) Dizideki üçüncü fenomenin (2. dizin) takipçi (followers) sayısı
console.log(
  `dizinin üçüncü ojesinin takipçi sayısı: ${fenomenler[2].followers}`
);
...
console.log(`Justin Bieber' in soyadı düzeltildi: ${fenomenler[6].profile}`);
...
function indekseGoreFenomen(arr, i) {
  return `${i}. indekste bulunan fenomen: ${arr[i].profile}`;
}
...
const k = fenomenSil(fenomenler, 2);
console.log("görev 5: ", k[2]);
...
function fenomenEkle(arr, value1, value2, value3, value4, value5) {
  const copy_array = [];
  for (let i = 0; i < arr.length; i++) {
    copy_array[i] = arr[i];
  }
  ...
  const new_object = {};
  new_object.number = value1;
  new_object.profile = value2;
  new_object.followers = value3;
  new_object.posts = value4;
  new_object.platform = value5;

  copy_array.push(new_object);

  return copy_array;
}

let ekli = fenomenEkle(fenomenler, 6, "Woekintech", 1000, 2022, "Instagram");
console.log("görev 6: ", ekli[ekli.length - 1]);
...
function enFenomenler(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].followers > 100000000) {
      higherFollowers.push(arr[i].profile);
    }
  }
  return higherFollowers;
}
...
function fenomenGonderimSayisi(arr, profileName) {
  let gonderimSayısı = 0;

  for (let i = 0; i < arr.length; i++) {
    if (arr[i].profile === profileName) {
      gonderimSayısı = arr[i].posts;
    }
  }
  return gonderimSayısı;
}
...
function platformaGoreCokGonderiYapanFenomen(arr, plaformName) {
  let platformFilterProfile = [];
  let postNumber = [];

  for (let i = 0; i < arr.length; i++) {
    if (arr[i].platform === plaformName) {
      platformFilterProfile.push(arr[i].profile);
      postNumber.push(arr[i].posts);
    }
  }
  console.log("platformFilter:", platformFilterProfile);
  console.log("postNumber:", postNumber);

  const max = Math.max(...postNumber);
  console.log("max :", max);

  const indexNumber = postNumber.indexOf(max);
  console.log("indexNumber:", indexNumber);

  return platformFilterProfile[indexNumber];
}
